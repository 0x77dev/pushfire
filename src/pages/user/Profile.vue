  ...
  <q-page v-if="currentUser">
    ...
    <div class="profile-container">
        ...
        <div class="user-info q-mt-lg">
            ...
            <div class="row justify-between">
              <label class="text-body1 text-left">User ID:</label>
              <pre class="text-body1 text-right" v-html="getUserData('id')"></pre>
            </div>
            <div class="row justify-between">
              <label class="text-body1 text-left">FCM Token:</label>
              <q-input v-model="fcmToken" label="FCM Token" stack-label dense />
            </div>
        </div>
    </div>
    <q-btn class="fixed-bottom-right q-ma-md" round="round" color="primary" icon="person" @click="setEditUserDialog(true); setBlur()"></q-btn>
...
export default {
  ...
  async mounted () {
    ...
    localStorage.notificationsToken = await this.askForPermissioToReceiveNotifications()
  },
  computed: {
    ...
    async askForPermissioToReceiveNotifications () {
      try {
        ...
        await this.updateUserData({
          fcmTokens: [...this.currentUser.fcmTokens, token]
        })
        this.$q.notify({
          message: `Your device fcm token is: ${token}`
        })
        return token
      } catch (error) {
        console.error(error)
